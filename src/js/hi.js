...
class DrawPlug {
  ...
  startDrawLine() {
    ...
    function onClick(e) {
      ...
      if (that.DRAWPOLYLINEPOINTS.length > 1 && that.ISMEASURE) { //是否是量距
        ...
        let myIcon_cust = L.divIcon({
          ...
          html: "<div>" + "<p>" + (that.MEASURERESULT / 1000).toFixed(2) + "KM</p>" + "</div>",
          iconSize: 26
        });
        let c = new L.Marker(that.DRAWMOVEPOLYLINE.getCenter(), { draggable: false, icon: myIcon_cust });
    ...
    function onDoubleClick(e) {
      ...
      var myIcon = L.divIcon({
        // html: (distance / 1000).toFixed(2) + "公里",
        className: 'my-div-icon',
        html: "<div style='background-color: #3a8ee6'>" + "<p style='font-size: 12px'>" + (distance / 1000).toFixed(2) + "KM</p>" + "</div>",
        iconSize: 26
      });
  ...
  startDrawPolygon(type) {
    ...
    that.map.on('dblclick', function (e) {
      ...
      for (let i = 0; i < as.length - 1; i++) {
        ...
        let myIcon_cust = L.divIcon({
          className: 'my-div-icon',
          html: "<div>" + "<p style='font-size: 12px'>" + (distance_cust / 1000).toFixed(2) + "KM</p>" + "</div>",
          iconSize: 26
        });
        let site_c = L.polyline([as[i], as[i + 1]], { color: 'rgba(231,8,8,0)', }).addTo(that.measureGroup);
      ...
      var myIcon = L.divIcon({
        html: "<div>" + "<p style='font-size: 12px'>" + (distance / 1000000).toFixed(3) + "KM²</p>" + "</div>",
        className: 'my-div-icon',
        iconSize: 26
      });
  ...
  markerToMap(marker) {
    this.measureGroup.addLayer(marker);
  }
  removeMarker(marker) {
    this.measureGroup.removeLayer(marker);
  }
}
